...
function UIcompositionClickNewLocal() {
	( !DAW.compositionNeedSave()
		...
		: gsuiPopup.confirm( "Предупреждение", "Вы уверены, что хотите удалить несохраненные работы?" )
...
function UIcompositionClickDelete( id ) {
	...
	gsuiPopup.confirm( "Предупреждение",
		`Вы уверены, что хотите удалить "${ cmp.name }" ? (нельзя отменить)`,
		"Удалить"
	).then( b => {
		if ( b ) {
			if ( id === DAW.get.id() ) {
...
function UIcompositionClickSave() {
	if ( document.cookie.indexOf( "cookieAccepted" ) > -1 ) {
		...
		gsuiPopup.alert( "Ошибка",
			"Вы должны принять файлы cookie, прежде чем локально сохранять свою композицию." );
	}
}
