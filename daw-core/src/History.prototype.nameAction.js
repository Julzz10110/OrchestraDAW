...
DAWCore.History._nameAction_synth = function( cmp, r, u ) {
	if ( r.synths ) {
		...
		if ( !rSyn || !uSyn ) {
			return rSyn
				? { i: "add", t: `Новый синтезатор "${ rSyn.name }"` }
				: { i: "remove", t: `Удалить синтезатор "${ uSyn.name }"` };
		}
		if ( "name" in rSyn ) {
		...
		if ( rSyn.oscillators ) {
			...
			if ( !rOsc || !uOsc ) {
				return rOsc
					? { i: "add", t: msg + "Новый осциллятор" }
					: { i: "remove", t: msg + "Удалить осциллятор" };
			}
		...
		if ( rSyn.envelopes ) {
			...
			return {
				i: "envelope",
				t: cmp.synths[ synthId ].name + `: изменить ${ envName } ${ attRel } конвертировать`,
			};
		}
	}
}
DAWCore.History._nameAction_blocks = function( cmp, r, u ) {
	...
	for ( const id in rBlcs ) {
		...
		if ( !rBlc )              { return { i: "erase", t: "Удалить" + msg }; }
		if ( !u.blocks[ id ] )    { return { i: "paint", t: "Добавить" + msg }; }
		if ( "duration" in rBlc ) { return { i: "crop", t: "Обрезать" + msg }; }
		if ( "when" in rBlc || "track" in rBlc ) { return { i: "move", t: "Переместить" + msg }; }
		if ( "selected" in rBlc ) { return rBlc.selected
			? { i: "selection ico--plus", t: "Выбрать" + msg }
			: { i: "selection ico--minus", t: "Отменить выбор" + msg };
		}
	}
}
DAWCore.History._nameAction_tracks = function( cmp, r, u ) {
	...
	if ( o ) {
		...
		for ( a in o ) {
			if ( o[ a ].name ) {
				return { i: "name", t: `Название трека: "${ u.tracks[ a ].name }" -> "${ o[ a ].name }"` }
			}
			if ( i++ ) {
				break;
			}
		...
		return i > 1
			? { i: "unmute", t: `Un/mute несколько треков` }
...
DAWCore.History._nameAction_pattern = function( cmp, r, u ) {
	for ( const id in r.patterns ) {
		...
		if ( !rpat || !upat ) {
			return rpat
				? { i: "add", t: `Новый паттерн "${ rpat.name }"` }
				: { i: "remove", t: `Удалить паттерн "${ upat.name }"` };
		}
		if ( rpat.synth ) {
			return { i: "param", t: `${ pat.name }: изменить синтезатор` };
		}
		if ( "name" in rpat ) {
			return { i: "name", t: `${ upat.name }: изменено на "${ rpat.name }"` };
		}
	}
}
